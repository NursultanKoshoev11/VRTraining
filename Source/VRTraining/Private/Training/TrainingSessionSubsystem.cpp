...
void UTrainingSessionSubsystem::StartTrainingSession(const FString& TraineeId, const FString& ModuleCode, const FString& ModuleVersion, const FString& DeviceId, const FString& AppVersion, const FString& Language)
{
    ...
    SessionStartedAt = FDateTime::UtcNow();
...
void UTrainingSessionSubsystem::RecordTrainingEvent(const FTrainingEventRecord& EventRecord)
{
    ...
    NormalizedEvent.TimeOffsetSeconds = FMath::Max(0, static_cast<int32>((FDateTime::UtcNow() - SessionStartedAt).GetTotalSeconds()));
...
void UTrainingSessionSubsystem::CompleteTrainingSession()
{
    if (CurrentSession.Status != ETrainingSessionStatus::InProgress)
    {
        return;
    ...
    CurrentSession.DurationSeconds = FMath::Max(0, static_cast<int32>((FDateTime::UtcNow() - SessionStartedAt).GetTotalSeconds()));
